    ...
    public class Hero : IGameObject, IHeroInterface, ISubject
    {
        ...
        { 
            foreach (var o in _observers) 
                o.OnNotify(CoinsRemaining, _hasPowerUp, _powerUpTimer, Lives);
        }
        public void CollectCoin() => CoinsRemaining--;
        public void EnableSuperJump() { 
            JumpStrategy = new SuperJump(new NormalJumpStrategy());
            _hasPowerUp = true;
            _powerUpTimer = 2.0f; 
        }
        ...
        public void Draw(SpriteBatch sb) { 
            if (_isVisible)
            {
                sb.Draw(Animatie.Texture, Position, Animatie.SourceRect, Color.White, 0f, Vector2.Zero, 1f, Animatie.Flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0f);
            }
